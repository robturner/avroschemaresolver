...
public abstract class Encoder implements Flushable {
   ...
   * whose second branch is a record "Rec" consisting of an Long field and
   * a Boolean field.  Your code would look something like this:
   * <pre>
   * out.writeIndex(1, "Rec");
  ...
  /** Call this method to write the tag of a union for encoders
   * that need the string representation.
   *
   * As an example of usage, let's say you want to write a union,
   * whose second branch is a record "Rec" consisting of an Long field and
   * a Boolean field.  Your code would look something like this:
  ...
  /**
   * Call this method to start a new union.
   *  <pre>
   *  out.writeUnionStart();
   *  out.writeIndex(index);
   *  out.writeUnionEnd();
   *  </pre>
   *  @throws AvroTypeException If not currently inside a union.
   */
  public void writeUnionStart() throws IOException {
  }

  /**
   * Call this method to terminate the current union.
   *  <pre>
   *  out.writeUnionStart();
   *  out.writeIndex(index);
   *  out.writeUnionEnd();
   *  </pre>
   *  @throws AvroTypeException If this is a stateful writer and a
   *          union is not expected
   */
  public void writeUnionEnd() throws IOException {
  }

  /**
   * Call this method to start writing a new record.
   * @throws AvroTypeException If this is a stateful writer and
   *         record is not the type of the next value to be write
   */
  public void writeRecordStart() throws IOException {
  }

  /**
   * Call this method to end the current record.
  ...
  /**
   * Writes an enumeration.
   * @param e int enum index
   * @param eTag String enumeration tag
   * @throws AvroTypeException If this is a stateful writer and an enumeration
   * is not expected or the <tt>e</tt> is out of range.
   * @throws IOException
   */
  ...
  /**
   * Call this method to start writing a new field.
   * @throws AvroTypeException When called outside of a
   *         field context
   */
  ...
  /**
   * Call this method to write a map key.
   * @throws AvroTypeException When called outside of a
   *         Map context
   */
