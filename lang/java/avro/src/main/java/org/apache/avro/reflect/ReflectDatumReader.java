...
public class ReflectDatumReader<T> extends SpecificDatumReader<T> {
   ...
   * representations.*/
  protected Object readArray(Object old, Schema expected, Decoder in)
      throws IOException {
  ...
  private Object readPrimitiveArray(Object array, Class<?> c, long l, Decoder in)
      throws IOException {
  ...
  private Object readCollection(Collection<Object> c, Schema expectedType,
      long l, Decoder in) throws IOException {
    do {
      for (int i = 0; i < l; i++) {
        Object element = read(null, expectedType, in);
        c.add(element);
  ...
  @Override
  protected void readField(Object oldDatum, Object record, Field f, 
    ...
    super.readField(oldDatum, record, f, in, state);
  }
}
